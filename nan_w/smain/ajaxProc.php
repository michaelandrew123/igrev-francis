...
}else if ($type == "wiki_save")
{
    if($_SESSION['mb_id']){
        ...
        sql_query(" insert into nan_quiz set mb_id = '{$_SESSION['mb_id']}', quiz_subject = '{$quiz_subject}',quiz_desc = '{$quiz_desc}',quiz_contents = '{$quiz_contents}',file_id = '{$file_id}', reg_date = now(), status='MAKE',quiz_type='4' ", false);
        $_SESSION['mb_memo'] = $mb_memo;
        $jsonContent = array(
            'code' => '200',
            'msg' => '저장 되었습니다.',
...
}else if ($type == "quiz_save")
{
    if($_SESSION['mb_id']){
        ...
        sql_query(" insert into nan_quiz set mb_id = '{$_SESSION['mb_id']}', quiz_subject = '{$quiz_subject}',quiz_desc = '{$quiz_desc}', reg_date = now(), status='MAKE',quiz_type='1' ", false);
...
}else if($type == "get_page_random"){
    $data = array();
    $listCnt = 40;
    $sql = " select *,a.quiz_idx quiz_idx,FN_CODE('quiz_type',a.quiz_type) quiz_type_name  from nan_quiz a left join nan_quiz_data_file b on b.file_idx = a.file_id  where a.del_yn = 'n'  ORDER BY RAND()  limit  ".$listCnt." ";
    $result = sql_query($sql);
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        //    $data[$i] = $row;
        array_push($data, $row);
