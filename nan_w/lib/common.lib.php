...
function push_send($device_id_arr, $subject, $content, $sub_data='', $os){
    global $g5;
	$headers = array('Content-Type:application/json ; charset=UTF-8', 'Authorization:key=AAAAnHcwh6E:APA91bFiS7Ol2zl1ElEgFpXYUp4XjxYK9FGr30Qaqz_Kp9LKFNV6QYLcIfgcyN_lbAWmVG7-j9rqMC8ADF0aYm29TCj1XvFcAIPYIqD7gMYdnvQ_TukbGLDHco7OB-qjEgi4atWhLhqT');    
    $arr   = array();
    
    if($os == 'ios'){
        $arr = array(
            "to"                => array($device_id_arr),
            "content-available" => true,
            "mutable_content"   => true,
            "priority"          => "high",
            "data"              => array('data'=>array('msg'=>'aaa'),'img_path'=>'https://naanna.kr/img/favicon-96x96.png','link'=>$sub_data ,'click_action'=>'FCM_EXE_ACTIVITY','media_type'=>'image'),
            "notification"      => array('title'=>$subject,'body'=>$content,'link'=>$sub_data,'click_action'=>'FCM_EXE_ACTIVITY', 'sound'=>1, 'vibrate'=>1 ,'badge'=>1)
        );
    } else {
        $arr['data'] = array();
        $arr['data']['link'] = $sub_data;
        $arr['data']['message'] = $content;
        $arr['notification'] = array('title'=> $subject,'body'=> $content,'link'=>$sub_data,'click_action'=>'FCM_EXE_ACTIVITY', 'sound'=>'default');
        
        /*푸쉬 대상자 최대 1000명*/
        $arr['registration_ids'] = array($device_id_arr);
    }
    ...
    sql_query("insert into push_log set os = '".$os."', to_ken = '".implode(",",array($device_id_arr))."', subject = '".$subject."', content = '".$content."', response = '".json_encode($response)."', pl_datetime = NOW()");
...
function push_send_nan_mb_msg($mb_id, $title="나안나", $msg, $url="naanna.kr", $type, $pu_id, $lrn_hist_no){
    if ( $mb_id )
    {
        ...
        //회원 중 조건에 맞는 회원 검색
        $sql = "select * from push_user_device where mb_id = '".$mb_id."' and os != '' and token != '' "; //mb_level = 2 and
        $rs = sql_query($sql);
        
        if($pu_id){
            $pu_data = sql_fetch("select * from push_data where pu_id = '".$pu_id."'");
            $title = $pu_data['pu_subject'];
            $msg = $pu_data['pu_content'];
            $url = $pu_data['pu_url'];
        } //else if($msg){}
        ...
        while($row = sql_fetch_array($rs)){
            //Push 발송 처리          
            $res = push_send($row['token'], $title, $msg, $url, $row['os']);
          //  print_r2($res);
            if($res->success)
                    $pu_success_count++;
        ...
        if($type=="learning"){
            $sql = "update nan_learning_hist
                    set     push_send_dt    = NOW()
                            ,push_send_yn   = 'y'
                    where lrn_hist_no = '{$lrn_hist_no}' ";
            sql_query($sql);
        } else {
...
//스케줄에서 리턴값없이 사용
function push_send_mb_sch($pu_id, $mb_id){
    //  alert($pu_id." id : ".$mb_no);
    if ($pu_id && $mb_id)
    {
        ...
        while($row = sql_fetch_array($rs)){
            //Push 발송 처리          
            $res = push_send($row['token'], $pu_data['pu_subject'], $pu_data['pu_content'], $pu_data['pu_url'], $row['os']);
          //  print_r2($res);
            if($res->success)
                    $pu_success_count++;
...
/* json 결과 response */
function jsonResp($resultCd="F9998", $resultMsg="서버 오류가 발생 하였습니다.", $resp=array()){
    $resp['resultCd'] = $resultCd ? $resultCd : "300" ;
    $resp['resultMsg'] = $resultMsg ? $resultMsg : "서버 오류가 발생 하였습니다." ;
...
/*
    ...
    api 결과 리턴시 이용
    메세지 및 코드 값 등록 하여 사용
    ex) S 로 시작시 정상, resultCd = 'S0001', resultMsg = "정상 처리 되었습니다.";
    ex) F 로 시작시 오류, resultCd = 'F0001', resultMsg = "서버 오류가 발생 하였습니다.";
*/
include_once("common.api.msg.php");
function setRespResult($type, $data=array(), $result=array()){
    $resultData = array();
    while ( $row = sql_fetch_array($result) ) { $resultData[]=$row; }
    $data['resultData']= $resultData;
    return jsonResp($type, getMsg($type), $data);
}

//비회원 가입자 일 때는 시스템 번호 0 이용
function getMbNoQry($mbId){
    if($mbId){
        $sql = " SELECT mb_no FROM g5_member where mb_id = '{$mbId}' ";
        $row = sql_fetch($sql);
        return $row['mb_no'] ? $row['mb_no'] : '0';
    } else {
        return '0';
    }
}
...
function getScrtyCd(){
    return substr(date("Y-m-d H:i:s") . "_" . hash("sha256", date("Y-m-d H:i:s").mt_rand(1, 10000)), 45);
}

/**
 * ********************************************************************
 * input 필터
 * ex) ifilter("변수값", "기본값", '변수타입', "최대길이(0일경우 제한없음)", "값이없을때 기본값(숫자형일 경우 기본값은 0")
 * *******************************************************************
 */
function ifilter($pKey, $pDefault = "", $pType = "STRING", $pLen = 0) {
    if ( empty($pKey) )     return null;
    if ( !isset($_REQUEST[ $pKey ]) && !empty($pDefault) ){
        $_REQUEST[ $pKey ] = $pDefault;
    }
    global $sh_data;
    if( $sh_data[$pKey] ){   return  $sh_data[$pKey];  }
    $pStr = $_REQUEST[ $pKey ];
    $result = "";
 ...
 //           $result = str_ireplace ( "iframe ", "ｉframe ", $result );
  ...
  //          $result = str_ireplace ( "embed", "ｅmbed ", $result );
    ...
    $_REQUEST[ $pKey ] = $result;
    return $result;
}
include_once("common.nan.query.php");